  ...
  <div>
    ...
    >
      <span class="grow flex items-center gap-2">
        ...
        ></div>
        {{ selected ? selected.fields.Title : 'Tous les rôles' }}
      </span>
...
const selected = computed(() => {
  return roles.find((role) => role.id === model.value);
});

const rolesFiltered = computed(() => {
  return roles.sort((a, b) => a.fields.Title.localeCompare(b.fields.Title));
});
